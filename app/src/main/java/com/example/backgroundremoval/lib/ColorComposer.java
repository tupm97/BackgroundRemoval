...
class ColorComposer extends BlendComposer {
    /**
     * Create a HueComposer.
     */
    public ColorComposer(int width, int height, int[] src, int[] dst) {
    ...
    public int[] compose() {
        ...
        for (int row = 0; row < height; row++) {
            for (int col = 0; col < width; col++) {
                ...
                ColorUtilities.HSLtoRGB(srcHSL[0], srcHSL[1], dstHSL[2], colorResults);
